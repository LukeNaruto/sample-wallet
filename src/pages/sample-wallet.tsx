...
import NoahToken from './noah-token';
...
const WalletCtx = createContext<IWallCtx>({} as IWallCtx);
...
function Wallet() {
    ...
    useEffect(() => {
        ...
        setWalletProvider(
            new ethers.providers.Web3Provider(window.web3.currentProvider as any)
        );
    }, []);
...
function Connect() {
    ...
    const connectToMetamask = async () => {
        try {
            ...
            console.log(2,accounts[0], network, ethers.utils.formatEther(balance));
        } catch(error){
            console.log('connectToMetamask**',error);
...
function Transfer() {
    ...
    return (
        <div className="flex flex-col gap-4 mt-4">
            <div className="text-4xl font-bold">Transfer-转以太</div>
            {transferring ? (
